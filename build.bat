...
SET TortoiseSvnVersion=version-1.11.0
...
NAnt.exe -buildfile:%~dp0\build.xml -logfile:build32.log -D:arch=x86 -D:TortoiseSvnVersion=%TortoiseSvnVersion% || GOTO END

call "C:\Program Files (x86)\Microsoft Visual Studio\2017\Community\VC\Auxiliary\Build\vcvars64.bat" 8.1
NAnt.exe -buildfile:%~dp0\build.xml -logfile:build64.log -D:arch=x64 -D:TortoiseSvnVersion=%TortoiseSvnVersion% || GOTO END
